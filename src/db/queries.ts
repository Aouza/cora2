...
export const exemploDeUso = async () => {
  console.log("🚀 Exemplos de uso do Drizzle ORM com Cora.Deep");

  try {
    // 1. Criar um usuário
    const novoUsuario = await criarUsuario("maria@exemplo.com");
    console.log("✅ Usuário criado:", novoUsuario);

    // 2. Inserir um relato
    const novoRelato = await inserirRelato(
      "Hoje foi um dia difícil, mas sinto que estou evoluindo...",
      novoUsuario.id
    );
    console.log("✅ Relato criado:", novoRelato);

    // 3. Adicionar reações simbólicas
    await adicionarEco(novoRelato.id, "🌱");
    await adicionarEco(novoRelato.id, "🫂");
    await adicionarEco(novoRelato.id, "🌱");
    console.log("✅ Ecos adicionados");

    // 4. Contar ecos
    const contadorEcos = await contarEcosPorRelato(novoRelato.id);
    console.log("✅ Contadores de ecos:", contadorEcos);

    // 5. Buscar relatos do usuário
    const relatosUsuario = await buscarRelatosDoUsuario(novoUsuario.id);
    console.log("✅ Relatos do usuário:", relatosUsuario.length);

    // 6. Criar um relatório
    const novoRelatorio = await criarRelatorio(
      novoUsuario.id,
      "Análise emocional profunda baseada nos últimos relatos...",
      "profundo"
    );
    console.log("✅ Relatório criado:", novoRelatorio);
  } catch (error) {
